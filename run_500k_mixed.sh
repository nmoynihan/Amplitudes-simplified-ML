...
# SQED CSV to amalgamate directly into training alongside the generated data.
# Expected to be a raw (infix) CSV with columns (simple, scrambled).
SQED_INJECT_CSV="${SQED_INJECT_CSV:-data/sqed_4pt_oneshot_100k.csv}"
SQED_INJECT_TOK="${SQED_INJECT_TOK:-data/sqed_4pt_oneshot_100k_tok.csv}"
COMBINED_TOK_OUT="${COMBINED_TOK_OUT:-data/combined_500k_tok.csv}"

# SQED-compatible broad default profile.
# SQED_COVER_PERCENT reduced to 15 (from 500k's 30) since real SQED data is
# injected directly; BROAD_PERCENT raised by 15 to compensate.
JOBS="${JOBS:-8}"
...
BROAD_PERCENT="${BROAD_PERCENT:-75}"
SQED_COVER_PERCENT="${SQED_COVER_PERCENT:-15}"
...
RUN_NAME="${RUN_NAME:-unit_500k_mixed}"
EPOCHS="${EPOCHS:-45}"
BATCH_SIZE="${BATCH_SIZE:-64}" # Adjust based on GPU memory; 64 works on my 5070 with AMP and dynamic padding.
...
HEAD_FF_DIM="${HEAD_FF_DIM:-2048}"
DROPOUT="${DROPOUT:-0.1}"
LEARNING_RATE="${LEARNING_RATE:-3e-4}"
GRAD_CLIP="${GRAD_CLIP:-1.0}"
LABEL_SMOOTHING="${LABEL_SMOOTHING:-0.1}"
AMP="${AMP:-1}"
AMP_DTYPE="${AMP_DTYPE:-bf16}" # Useful on my 5070 GPU, set to fp16 for better compatibility
...
mkdir -p "$(dirname "$COMBINED_TOK_OUT")"
...
echo "Training cap: SAMPLES=$SAMPLES (generated) + SQED inject: $SQED_INJECT_CSV"
echo "COMBINED_TOK_OUT=$COMBINED_TOK_OUT"
echo "TRAIN_SAMPLE_SEED=$TRAIN_SAMPLE_SEED"
...
# Tokenise a raw (infix) CSV into a tokenised CSV using gen_data's tokenise_csv.
tokenise_inject_csv() {
  local src="$1"
  local dst="$2"
  python3 - "$src" "$dst" "$TOKENIZER_MAX_PARTICLES" "$MAX_TOKENS" <<'PY'
import sys
from pathlib import Path
...
# Combine two tokenised CSVs, shuffle, and write to a third path.
combine_tok_csvs() {
  local gen_tok="$1"
  local inject_tok="$2"
  local out="$3"
  ...
  python3 - "$gen_tok" "$inject_tok" "$out" "$seed" <<'PY'
import csv
import random
import sys
from pathlib import Path

gen_tok, inject_tok, out, seed = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4])
...
rows_gen    = read_rows(gen_tok)
rows_inject = read_rows(inject_tok)
all_rows = rows_gen + rows_inject
...
with open(out, "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=["simple", "scrambled"])
    writer.writeheader()
    writer.writerows(all_rows)

print(
    f"Combined {len(rows_gen)} generated + {len(rows_inject)} injected = "
    f"{len(all_rows)} total rows -> {out}",
    file=sys.stderr,
)
PY
}
...
if [[ "$DO_TRAIN" -eq 1 ]]; then
  if truthy "$SKIP_DATA_GEN"; then
    echo "Skipping data generation and combination; using existing combined CSV: $COMBINED_TOK_OUT"
    if [[ ! -f "$COMBINED_TOK_OUT" ]]; then
      echo "Combined tokenised CSV not found: $COMBINED_TOK_OUT"
      echo "Set COMBINED_TOK_OUT=/path/to/combined_tok.csv or disable SKIP_DATA_GEN."
      exit 1
    fi
  else
    echo "Generating ${SAMPLES} training examples with gen_data.sh profile..."
    env \
    ...
    # Tokenise the SQED inject CSV if a tokenised copy doesn't already exist.
    SQED_INJECT_TOK_ABS="$(abspath "$SQED_INJECT_TOK")"
    if [[ ! -f "$SQED_INJECT_TOK_ABS" ]]; then
      echo "Tokenising SQED inject CSV: $SQED_INJECT_CSV -> $SQED_INJECT_TOK"
      if [[ ! -f "$SQED_INJECT_CSV" ]]; then
        echo "SQED inject CSV not found: $SQED_INJECT_CSV"
        echo "Set SQED_INJECT_CSV= to point at the raw SQED training data."
        exit 1
      fi
      tokenise_inject_csv "$(abspath "$SQED_INJECT_CSV")" "$SQED_INJECT_TOK_ABS"
    else
      echo "Using existing tokenised SQED inject CSV: $SQED_INJECT_TOK"
    fi

    # Combine the generated data with the SQED inject data and shuffle.
    echo "Combining generated tok CSV with SQED inject tok CSV..."
    combine_tok_csvs \
      "$(abspath "$TOK_OUT")" \
      "$SQED_INJECT_TOK_ABS" \
      "$(abspath "$COMBINED_TOK_OUT")" \
      "$TRAIN_SAMPLE_SEED"
  fi

  TRAIN_ARGS=(
    --run_name "$RUN_NAME"
    --data-files "$(abspath "$COMBINED_TOK_OUT")"
    --epochs "$EPOCHS"
