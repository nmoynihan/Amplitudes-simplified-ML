'''Paolo's M2 variant of transformer_trainer.py — adds Apple-GPU (MPS) support + M2 defaults.

Identical to transformer_trainer.py except: (1) device selection prefers CUDA, then MPS, then CPU;
(2) M2-tuned argparse defaults. The base model/train_step already honour model.device and the
nested-tensor fast path is disabled, so MPS runs in fp32 (autocast/scaler/pin_memory stay cuda-gated).
'''
import os
...
os.environ.setdefault("PYTORCH_ENABLE_MPS_FALLBACK", "1")  # MPS: CPU fallback for any unsupported op
...
import transformer_functions as _tf

# --- MPS memory fix (this file only; collaborator's transformer_functions.py untouched) ---
# Variable-length batches (bucketing + dynamic_padding) make MPS's caching allocator pile up
# size-mismatched freed blocks until it OOMs even for a tiny model. train_model() resolves
# `train_step` as a module global at call time, so we wrap that global here to release the MPS
# cache after every batch. Active only on MPS; CUDA/CPU paths are byte-for-byte unchanged.
_MPS_ACTIVE = False
_orig_train_step = _tf.train_step


def _train_step_mps(*args, **kwargs):
...
_tf.train_step = _train_step_mps
...
def parse_args():
    ...
    parser.add_argument("--epochs", type=int, default=60)        # M2: early stopping usually cuts this
    parser.add_argument("--batch-size", type=int, default=32)    # M2 default
    ...
    parser.add_argument("--embedding-dim", "--embedding_dim", dest="embedding_dim", type=int, default=384)  # middle-size
    parser.add_argument("--n-heads", "--n_heads", dest="n_heads", type=int, default=6)
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device("mps")          # Apple GPU (M-series)
    else:
        device = torch.device("cpu")
    print(f"Using device: {device}")
    global _MPS_ACTIVE
    _MPS_ACTIVE = device.type == "mps"   # arms the per-batch torch.mps.empty_cache() wrapper
    if device.type == "cuda":
