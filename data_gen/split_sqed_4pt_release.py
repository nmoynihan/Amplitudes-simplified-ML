#!/usr/bin/env python3
"""Build a verified 499,800/200 release from a 4PT SQED candidate pool.
...
``generate_sqed_4pt_500k.sh``.  Its 10% headroom allows duplicate rows to be
...
DEFAULT_SPLIT_SEED = 4_020_004
DEFAULT_MAX_TOKENS = 4096
DEFAULT_TOKENIZER_MAX_PARTICLES = 8
DEFAULT_PROGRESS_EVERY = 50_000
SQED_4PT_ASSUMPTIONS = OnShellAssumptions(
    massless_momenta=frozenset({2, 3}),
    transverse_field_strengths=frozenset({2, 3}),
)

REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_STAGING_DIR = REPO_ROOT / "data" / "sqed" / "sqed_4pt_500k_staging"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "data" / "sqed" / "sqed_4pt_500k"
DEFAULT_RAW = DEFAULT_STAGING_DIR / "sqed_4pt_oneshot_candidates_550000.csv"
DEFAULT_TOKENISED = (
    DEFAULT_STAGING_DIR / "sqed_4pt_oneshot_candidates_550000_tok.csv"
)
DEFAULT_GENERATION_LOG = (
    DEFAULT_STAGING_DIR / "sqed_4pt_oneshot_candidates_550000.log"
)
...
DEFAULT_PROFILE = {
    ...
    "onshell_assumptions": SQED_4PT_ASSUMPTIONS.to_dict(),
    ...
    "profile": {
        ...
        "sqed_cover": {
            ...
            "old_style_probability": 0.65,
            "spurious_repeat_probability": 0.15,
            "scalar_power_probability": 0.1,
            "scrambles": [
                "multiply_one",
                "ward",
                "momentum",
...
def _iter_release_rows(
    ...
    *,
    ...
    """Yield the first globally unique, unambiguous rows for each quota."""
    ...
    for index, raw_row, token_row in _aligned_rows(raw_path, tokenised_path):
        ...
        target_analysis = analyze_simple_expression(
            raw_row[0],
            assumptions=SQED_4PT_ASSUMPTIONS,
        )
...
def output_paths(
    ...
    *,
    ...
    return OutputPaths(
        train_raw=output_dir / f"sqed_4pt_oneshot_train_{train_rows}.csv.gz",
        train_tokenised=(
            output_dir / f"sqed_4pt_oneshot_train_{train_rows}_tok.csv.gz"
        ),
        test_raw=output_dir / f"sqed_4pt_oneshot_test_{test_rows}.csv.gz",
        test_tokenised=(
            output_dir / f"sqed_4pt_oneshot_test_{test_rows}_tok.csv.gz"
        ),
        manifest=(
            output_dir / f"sqed_4pt_oneshot_{release_rows}_manifest.json"
        ),
    )
...
def _code_hashes() -> dict[str, str]:
    paths = (
        ...
        REPO_ROOT / "generate_sqed_4pt_500k.sh",
    )
    return {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Create a verified target-disjoint 499,800/200 4PT SQED release "
...
def _validate_args(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.tokenizer_max_particles < 4:
        parser.error("--tokenizer-max-particles must be at least 4")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        "Created verified 4PT SQED release: "
