#!/usr/bin/env python3
"""
kinematics.py — Random N-point phase-space generator (all-gluon / massless).

    p_1 … p_N       : massless gluons (p_i² = 0)

All N legs are massless with Σ_i p_i = 0.   Signature (+,−,−,−).
Each leg carries a transverse polarisation ε_i with ε·p = 0, ε² = −1.

`E_scale` sets the overall energy magnitude (it is NOT a mass). The two
endpoint legs p_1, p_N are built via a two-body balance with `endpoint_mass`
(default 0 → massless); this is decoupled from `E_scale` so masslessness does
not collapse the energy sampling.
...
def generate_kinematics(
    ...
    E_scale: float = 2.0,
    *,
    endpoint_mass: float = 0.0,
    E_min: float | None = None,
    E_max: float | None = None,
    pol_mode: str = "coulomb",
    seed: int | None = None,
) -> tuple[np.ndarray, np.ndarray]:
    """Generate one random N-point all-gluon phase-space configuration.

    All N legs are massless (endpoint_mass=0) with Σp = 0; each carries a
    transverse polarisation ε_i.  `E_scale` is the energy magnitude (not a
    mass) and is decoupled from `endpoint_mass`.

    Returns
    -------
    momenta       : (N, 4) array   [p_1 … p_N]
    polarisations : (N, 4) array   [ε_1 … ε_N]
    """
    if E_min is None:
        E_min = 0.3 * E_scale
    if E_max is None:
    ...
    n_direct = N - 2  # legs sampled directly; the 2 endpoints balance them

    # 1) Sample n_direct massless momenta until the parent is timelike enough
    #    to split into two endpoints of mass `endpoint_mass`.
    s_min = max(4.0 * endpoint_mass ** 2, 1e-9)
    while True:
        energies = rng.uniform(E_min, E_max, size=n_direct)
        photons = np.stack([
    ...
    # 2) Two-body decay in the parent rest frame (massless endpoints by default)
    sqrt_s = np.sqrt(s)
    E_star = sqrt_s / 2.0
    p_mag = np.sqrt(E_star ** 2 - endpoint_mass ** 2)
    n_hat = _random_unit_vec(rng)

    p1_rest = np.array([E_star, *(p_mag * n_hat)])
    pN_rest = np.array([E_star, *(-p_mag * n_hat)])

    # 3) Boost endpoints so total momentum is +K (balances flipped direct legs)
    beta = K[1:] / K[0]
    p1 = _lorentz_boost(p1_rest, beta)
    pN = _lorentz_boost(pN_rest, beta)

    # 4) Flip direct legs → outgoing convention
    photons = -photons

    # 5) Assemble momenta in leg order [p_1 … p_N]
    momenta = np.vstack((p1, photons, pN))

    # 6) Polarisations for ALL N legs (every leg is a massless gluon)
    pols = np.stack([pol_fn(k4, rng) for k4 in momenta])
...
if __name__ == "__main__":
    ...
    def _check(N, E_scale, pol_mode="coulomb"):
        mom, pol = generate_kinematics(N, E_scale, pol_mode=pol_mode)
        msq = lambda v: v[0] ** 2 - np.dot(v[1:], v[1:])

        # all N legs massless
        for v in mom:
            assert abs(msq(v)) < 1e-10, f"leg not massless: {msq(v)}"

        tot = mom.sum(axis=0)
        assert np.allclose(tot, 0.0, atol=1e-10), f"momentum not conserved: {tot}"

        # one polarisation per leg, transverse and unit-normalised
        assert pol.shape == (N, 4), f"expected ({N}, 4) pols, got {pol.shape}"
        for k, eps in zip(mom, pol):
